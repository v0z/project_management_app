...
@router.get(
	"/", response_model=List[ProjectResponse], summary="Show all projects", status_code=status.HTTP_200_OK
)
async def list_all(
...
@router.post(
	"/", response_model=ProjectResponse, summary="Create project", status_code=status.HTTP_201_CREATED
)
async def create(
...
@router.patch(
	"/{project_id}", summary="Update project", response_model=ProjectResponse, status_code=status.HTTP_200_OK
)
async def update(
	project_id: UUID,
